...
function EventLibraryManagement({ toast, confirm, adminRole }) {
    ...
    const handleRemoveClick = async (eventId, eventName) => {
        if (await confirm({title: 'Are you sure?', message: `This will permanently delete "${eventName}" from the Event Library.`})) {
            try {
    ...
    const handleBulkLoad = async () => {
        const result = await confirm({
            ...
            message: "This will add any missing standard events to the library. It will not create duplicates.",
        });
        if (!result) return;

        if (typeof STANDARD_EVENT_LIBRARY === 'undefined' || !Array.isArray(STANDARD_EVENT_LIBRARY)) {
            toast.error("Standard event library is not defined in config.js.");
            return;
        }
        ...
        if (eventsToAdd.length === 0) {
            toast.error("All standard library events already exist.");
            return;
        }

        toast.success(`Adding ${eventsToAdd.length} new library events...`);
        const batch = writeBatch(db);
...
function ScheduleManagement({ toast, confirm, allMeets }) {
    ...
    const handleRemoveEvents = async (idsToRemove) => {
        ...
        const result = await confirm({title: 'Are you sure?', message: `This will remove ${idsToRemove.length} event(s) from this meet's schedule.`});
        if (result) {
            const batch = writeBatch(db);
    ...
    const handleRemoveAll = async () => {
        ...
        const result = await confirm({title: 'Are you sure?', message: "This will remove ALL events from this meet's schedule. This action cannot be undone."});
        if (result) {
            const batch = writeBatch(db);
...
function RosterManagement({ toast, confirm, adminRole }) {
    ...
    const handleRemoveClick = async (swimmerToRemove) => {
        const result = await confirm({
            title: 'Are you sure?',
            message: `This removes ${swimmerToRemove.firstName} from the team roster. It does NOT remove them from any events they are already entered in.`,
            confirmText: 'Yes, remove from roster'
        });
        if (!result) return;
...
function EntryManagement({ toast, confirm, adminRole, allMeets, selectedMeet }) {
    const isSuperAdmin = adminRole === 'SUPERADMIN';
    const [selectedMeetId, setSelectedMeetId] = useState("");
    const [managingTeam, setManagingTeam] = useState(isSuperAdmin ? "" : adminRole);
    const [roster, setRoster] = useState([]);
    ...
    const handleDeleteEntry = async (entry) => {
        const result = await confirm({
            title: 'Are you sure?',
            message: `Remove ${entry.swimmerName} from E${entry.eventNumber}: ${entry.eventName}?`,
    ...
    const handleAddOrUpdateSwimmerToEvent = async () => {
        ...
        if (manualHeat && manualLane) {
            if (manualLane > lanesAvailable) {
                toast.error(`Invalid lane. Only ${lanesAvailable} lanes are available for this meet.`);
                return;
            }
